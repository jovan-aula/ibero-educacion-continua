...
export default function App() {
  ...
  const confirmar = async (progId,modId)=>{
    ...
    try{
      ...
      const body="Estimado/a "+m.docente+",\n\nNos complace confirmar su participación como docente en el siguiente módulo:\n\nPrograma: "+p.nombre+"\nMódulo: "+m.numero+" — "+m.nombre+"\nFechas: "+fmtFecha(m.fecha_inicio)+" al "+fmtFecha(m.fecha_fin)+(m.horario?"\nHorario: "+m.horario:"")+"\nDías: "+(m.dias||[]).join(", ")+"\nTotal de horas: "+totalH+"h ("+m.clases+" clases de "+m.horas_por_clase+"h)\n\nAgradecemos su valiosa colaboración con nuestra comunidad universitaria.\n\nAtentamente,\nDirección de Educación Continua\nIBERO Tijuana";
      const html="<div style='font-family:Georgia,serif;max-width:620px;margin:0 auto'><div style='background:#C8102E;padding:24px 36px'><div style='color:#fff;font-size:32px;font-weight:900;letter-spacing:3px'>IBERO</div><div style='color:rgba(255,255,255,0.75);font-size:9px;letter-spacing:4px;font-family:system-ui'>TIJUANA</div><div style='color:rgba(255,255,255,0.7);font-size:11px;margin-top:6px;font-family:system-ui'>Dirección de Educación Continua</div></div><div style='padding:32px 36px;border:1px solid #e5e7eb;border-top:none'><div style='display:inline-block;background:#fef2f2;color:#C8102E;font-size:11px;font-weight:700;padding:4px 12px;border-radius:20px;margin-bottom:20px;font-family:system-ui'>DOCENTE CONFIRMADO</div><h2 style='font-size:20px;color:#1a1a1a;margin:0 0 20px'>"+m.nombre+"</h2><table style='width:100%;border-collapse:collapse;font-size:14px;font-family:system-ui'><tr style='border-bottom:1px solid #f3f4f6'><td style='padding:10px 0;color:#6b7280;width:130px'>Programa</td><td style='font-weight:600'>"+p.nombre+"</td></tr><tr style='border-bottom:1px solid #f3f4f6'><td style='padding:10px 0;color:#6b7280'>Docente</td><td style='font-weight:600'>"+m.docente+"</td></tr><tr style='border-bottom:1px solid #f3f4f6'><td style='padding:10px 0;color:#6b7280'>Período</td><td>"+fmtFecha(m.fecha_inicio)+" — "+fmtFecha(m.fecha_fin)+"</td></tr>"+(m.horario?"<tr style='border-bottom:1px solid #f3f4f6'><td style='padding:10px 0;color:#6b7280'>Horario</td><td>"+m.horario+"</td></tr>":"")+"<tr style='border-bottom:1px solid #f3f4f6'><td style='padding:10px 0;color:#6b7280'>Días</td><td>"+(m.dias||[]).join(", ")+"</td></tr><tr><td style='padding:10px 0;color:#6b7280'>Horas</td><td>"+m.clases+" clases · "+m.horas_por_clase+"h c/u · <strong>"+totalH+"h total</strong></td></tr></table><div style='color:#374151;font-size:14px;line-height:1.8;border-top:2px solid #C8102E;padding-top:20px;margin-top:20px;font-family:system-ui'>"+body.replace(/\n/g,"<br/>")+"</div></div><div style='background:#f9f9f9;padding:20px 36px;border:1px solid #e5e7eb;border-top:none;font-family:system-ui;font-size:12px;color:#6b7280;line-height:1.8'><strong style='color:#1a1a1a'>Dirección de Educación Continua · IBERO Tijuana</strong><br/>Av. Centro Universitario #2501, Playas de Tijuana, C.P. 22500<br/>Tel: 664 630 1577 Ext. 2576 · WhatsApp: 664 764 1119<br/>info@tijuana.ibero.mx<br/><br/><span style='font-size:10px;color:#9ca3af'>© 2026 IBERO Tijuana. Todos los derechos reservados.</span></div></div>";
      let ok=0;
      for(const dest of dests){
        try{
          const res=await fetch("https://services.leadconnectorhq.com/conversations/messages/outbound",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+notifCfg.notif_api_key,"Version":"2021-04-15"},body:JSON.stringify({type:"Email",locationId:notifCfg.notif_location_id,toEmail:dest.email,subject:"Confirmación de participación — "+m.nombre+" · "+p.nombre,html})});
          if(res.ok)ok++;
        }catch(e){}
      }
