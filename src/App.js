...
function ImportModal({prog,notifConfig,fieldMap,onImport,onClose}) {
  ...
  const doImport = () => {
    ...
    const toAdd = contacts.filter(c=>selected.includes(c.id)&&!existIds.has(c.id)).map(c=>{
      ...
      delete campos_extra["Empresa"];

      return {id:c.id,nombre:c.name||((c.firstName||"")+" "+(c.lastName||"")).trim(),email:c.email||"",telefono:c.phone||"",empresa,fuente:c.source||"",estatus:"activo",asistencia:{},campos_extra};
    });
...
export default function App() {
  ...
  const exportCSV = prog=>{
    const rows=ests(prog).map(e=>{
      ...
      Object.entries(e.campos_extra||{}).forEach(([k,v])=>{if(!base[k])base[k]=v||"";});
  ...
  const exportDocente = prog=>{
    const rows=ests(prog).map(e=>({Nombre:e.nombre||"",Empresa:e.empresa||"",Puesto:(e.campos_extra&&e.campos_extra["Puesto"])||e["Puesto"]||""}));
    if(!rows.length)return;
                      ...
                      <div key={e.id} style={{...S.card,border:"1px solid "+(riesgo?"#fca5a5":"#e5e7eb"),padding:"14px 18px"}}>
                        <div style={{display:"flex",alignItems:"flex-start",gap:12,flexWrap:"wrap"}}>
                          <div style={{flex:1,minWidth:200}}>
                            ...
                            {Object.keys(e.campos_extra||{}).length>0&&(
                              <div style={{marginTop:6,display:"flex",gap:6,flexWrap:"wrap"}}>
                                {Object.entries(e.campos_extra||{}).filter(([,v])=>v).map(([k,v])=>(
                                  <span key={k} style={{fontSize:11,background:"#f3f4f6",borderRadius:4,padding:"2px 8px",color:"#374151",fontFamily:"system-ui"}}>{k+": "+v}</span>
                                ))}
                              </div>
                            )}
                          </div>
